# coding=utf-8
"""
MindVision Camera SDK Python Wrapper (MVSDK)

This module provides Python bindings for the MindVision industrial camera SDK.
Implementation based on spec/llm.txt - MindVision Industrial Camera SDK Specification (v2.4)

Platform Support:
- Windows: MVCAMSDK.DLL (32-bit) / MVCAMSDK_X64.DLL (64-bit)
- macOS: libmvsdk.dylib (ARM64 M1+ and x86_64 Intel)
- Linux: libMVSDK.so

Critical Requirements (spec section 23.14):
✅ 1. Platform-specific SDK loading
✅ 2. CameraSdkInit(1) called before any operations
✅ 3. 16-byte aligned memory allocation (CameraAlignMalloc)
✅ 4. Proper buffer release (CameraReleaseImageBuffer)
...
Usage:
    ...
    # Initialize camera
    hCamera = mvsdk.CameraInit(cameras[0], -1, -1)
...
- spec/python_demo/cv_grab.py - OpenCV integration
    ...
    try:
        if is_win:
            ...
            if platform.system() == "Darwin":
                ...
                if arch == "arm64" and os.path.exists(bundled_macos_lib_arm64):
                    _sdk = cdll.LoadLibrary(bundled_macos_lib_arm64)
                elif arch in ("x86_64", "i386") and os.path.exists(bundled_macos_lib_x86):
                    _sdk = cdll.LoadLibrary(bundled_macos_lib_x86)
                else:
        ...
        _sdk = None
        CALLBACK_FUNC_TYPE = CFUNCTYPE
        print(f"Warning: MVSDK native library not loaded: {e}")
        return
...
CAMERA_STATUS_DEVICE_VEDIO_CLOSED = (
    -20
)  # 没有打开设备视频，调用录像相关的函数时，如果相机视频没有打开，则回返回该错误。
...
CAMERA_STATUS_GIGE_FILTER_INIT_ERROR = (
    -36
...
class tSdkCameraDevInfo(MvStructure):
    _fields_ = [
        ("acProductSeries", c_char * 32),  # 产品系列
...
class tSdkResolutionRange(MvStructure):
    _fields_ = [
        ...
        (
            "uSkipModeMask",
            c_uint,
        ),  # SKIP模式掩码，为0，表示不支持SKIP 。bit0为1,表示支持SKIP 2x2 bit1为1，表示支持SKIP 3x3....
        (
            "uBinSumModeMask",
            c_uint,
        ),  # BIN(求和)模式掩码，为0，表示不支持BIN 。bit0为1,表示支持BIN 2x2 bit1为1，表示支持BIN 3x3....
        (
            "uBinAverageModeMask",
            c_uint,
        ),  # BIN(求均值)模式掩码，为0，表示不支持BIN 。bit0为1,表示支持BIN 2x2 bit1为1，表示支持BIN 3x3....
...
class tSdkExpose(MvStructure):
    _fields_ = [
        ...
        (
            "fAnalogGainStep",
            c_float,
        ),  # 模拟增益每增加1，对应的增加的放大倍数。例如，uiAnalogGainMin一般为16，fAnalogGainStep一般为0.125，那么最小放大倍数就是16*0.125 = 2倍
        (
            "uiExposeTimeMin",
            c_uint,
...
class tSdkFrameEvent(MvStructure):
    _fields_ = [
        (
            "uType",
            c_uint,
        ),  # ///< \~chinese 事件类型(1:帧开始   2:帧结束) \~english Event type (1:frame start   2:frame end)
        (
            "uStatus",
            c_uint,
        ),  # ///< \~chinese 状态(0:成功  非0:错误) \~english Status (0:success, non-zero:error)
...
def CameraSetSysOption(optionName, value):
    err_code = _sdk.CameraSetSysOption(
        _str_to_string_buffer(optionName), _str_to_string_buffer(str(value))
    )
    SetLastError(err_code)
    return err_code
...
def CameraImageProcessEx(hCamera, pbyIn, pbyOut, pFrInfo, uOutFormat, uReserved):
    err_code = _sdk.CameraImageProcessEx(
        hCamera, c_void_p(pbyIn), c_void_p(pbyOut), byref(pFrInfo), uOutFormat, uReserved
    )
    SetLastError(err_code)
    return err_code
...
def CameraInitRecord(hCamera, iFormat, pcSavePath, b2GLimit, dwQuality, iFrameRate):
    err_code = _sdk.CameraInitRecord(
        hCamera, iFormat, _str_to_string_buffer(pcSavePath), b2GLimit, dwQuality, iFrameRate
    )
    SetLastError(err_code)
    return err_code
...
def CameraSaveImageEx(
    hCamera, lpszFileName, pbyImageBuffer, uImageFormat, iWidth, iHeight, byFileType, byQuality
):
...
def CameraSetImageResolutionEx(
    hCamera, iIndex, Mode, ModeSize, x, y, width, height, ZoomWidth, ZoomHeight
):
...
def CameraGetAeWindow(hCamera):
    ...
    err_code = _sdk.CameraGetAeWindow(
        hCamera, byref(piHOff), byref(piVOff), byref(piWidth), byref(piHeight)
    )
    SetLastError(err_code)
...
def CameraGetCrossLine(hCamera, iLine):
    ...
    err_code = _sdk.CameraGetCrossLine(
        hCamera, iLine, byref(px), byref(py), byref(pcolor), byref(pbVisible)
    )
    SetLastError(err_code)
...
def CameraCustomizeReferWin(hCamera, iWinType, hParent):
    ...
    err_code = _sdk.CameraCustomizeReferWin(
        hCamera, iWinType, hParent, byref(piHOff), byref(piVOff), byref(piWidth), byref(piHeight)
    )
    SetLastError(err_code)
    return (piHOff.value, piVOff.value, piWidth.value, piHeight.value)
...
def CameraCreateSettingPage(
    hCamera, hParent, pWinText, pCallbackFunc=None, pCallbackCtx=0, uReserved=0
):
...
def CameraGetImageBufferEx2(hCamera, pImageData, uOutFormat, wTimes):
    ...
    err_code = _sdk.CameraGetImageBufferEx2(
        hCamera, c_void_p(pImageData), uOutFormat, byref(piWidth), byref(piHeight), wTimes
    )
    SetLastError(err_code)
    if err_code != 0:
        raise CameraException(err_code)
...
def CameraGetCapabilityEx2(hCamera):
    ...
    pMaxHeight = c_int()
    pbColorCamera = c_int()
    err_code = _sdk.CameraGetCapabilityEx2(
        hCamera, byref(pMaxWidth), byref(pMaxHeight), byref(pbColorCamera)
    )
    SetLastError(err_code)
...
def CameraEvaluateImageDefinition(hCamera, iAlgorithSel, pbyIn, pFrInfo):
    ...
    err_code = _sdk.CameraEvaluateImageDefinition(
        hCamera, iAlgorithSel, c_void_p(pbyIn), byref(pFrInfo), byref(DefinitionValue)
    )
    SetLastError(err_code)
...
def CameraFlatFieldingCorrectSetParameter(
    hCamera, pDarkFieldingImage, pDarkFieldingFrInfo, pLightFieldingImage, pLightFieldingFrInfo
):
...
def CameraGetDenoise3DParams(hCamera):
    ...
    bUseWeight = c_int()
    Weights = (c_float * 8)()
    err_code = _sdk.CameraGetDenoise3DParams(
        hCamera, byref(bEnable), byref(nCount), byref(bUseWeight), Weights
    )
    SetLastError(err_code)
...
def CameraGetImageBufferPriorityEx2(hCamera, pImageData, uOutFormat, wTimes, Priority):
    ...
    err_code = _sdk.CameraGetImageBufferPriorityEx2(
        hCamera, c_void_p(pImageData), uOutFormat, byref(piWidth), byref(piHeight), wTimes, Priority
    )
    SetLastError(err_code)
    if err_code != 0:
        raise CameraException(err_code)
...
def CameraMultiEyeImageProcess(
    hCamera, iEyeIndex, pbyIn, pInFrInfo, pbyOut, pOutFrInfo, uOutFormat, uReserved
):
...
def CameraImage_BitBlt(Image, hWnd, xDst, yDst, cxDst, cyDst, xSrc, ySrc):
    err_code = _sdk.CameraImage_BitBlt(
        c_void_p(Image), c_void_p(hWnd), xDst, yDst, cxDst, cyDst, xSrc, ySrc
    )
    SetLastError(err_code)
    return err_code
